Given a connected undirected graph with n nodes and m edges, with each node having a distinct label from 0 to n-1, create a clone of the graph. Each node in the graph contains an 
integer val and an array (neighbors) of nodes, containing nodes that are adjacent to the current node.

Note: If the user returns a correct copy of the given graph, then the system will print 1; in the case when an incorrect copy is generated 
or when the user returns the original node, the system will print 0.
        ...
        while(!q.empty()){
            ...
            for(auto nei:curr->neighbors){
                if(mp.find(nei->val)==mp.end()){
                    Node *neighborNode=new Node(nei->val);
                    mp[nei->val]=neighborNode;
                    q.push(nei);
                }
                
               mp[curr->val]->neighbors.push_back(mp[nei->val]);
            }
        }
        return copy;
    }
