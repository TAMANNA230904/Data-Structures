...
{
    public:
  void solve(Node *root, int sum,int temp_sum,vector<vector<int>> &ans,vector<int> &temp){
      if(!root)
      return;
        ...
        solve(root->left,sum,temp_sum,ans,temp); 
        solve(root->right,sum,temp_sum,ans,temp); 
